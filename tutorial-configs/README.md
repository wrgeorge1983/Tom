...
- An existing Nautobot (or NetBox) instance with devices configured
- Network devices accessible via SSH from the Docker host
- API token for your Nautobot/NetBox instance
...
plugin_nautobot_credential_default: "default_creds"
```
...
## Nautobot/NetBox Setup Requirements

For Tom to work with your Nautobot/NetBox, devices need:
...
| `seed_netbox.py` | (Optional) Script to seed a local NetBox |
...
## Using NetBox Instead

If you're using NetBox instead of Nautobot, update `tom_controller_config.yaml`:

```yaml
inventory_type: "netbox"

plugin_netbox_url: "https://your-netbox.example.com"
plugin_netbox_token: "your-netbox-api-token"

# NetBox uses lowercase status values
plugin_netbox_status_filter: ["active"]

# etc. - see comments in the config file
```
