...
class TestGoogleIDTokenValidation:
    ...
    @pytest.mark.asyncio
    async def test_validate_recorded_google_id_token(self, load_recording, httpx_mock: HTTPXMock):
        ...
        httpx_mock.add_response(
            url=recording["jwks_request"]["url"],
            json=recording["jwks_response"]
        )
    ...
    @pytest.mark.asyncio
    async def test_google_discovery_request(self, load_recording, httpx_mock: HTTPXMock):
    ...
    @pytest.mark.asyncio
    async def test_expired_google_token_rejected(self, load_recording, httpx_mock: HTTPXMock):
...
class TestDuoIDTokenValidation:
    ...
    @pytest.mark.asyncio
    async def test_validate_recorded_duo_id_token(self, load_recording, httpx_mock: HTTPXMock):
...
class TestJWKSCaching:
    ...
    @pytest.mark.asyncio
    async def test_jwks_cached_on_second_request(self, load_recording, httpx_mock: HTTPXMock):
        ...
        jwks_requests = [r for r in requests if "jwks" in str(r.url) or "keys" in str(r.url)]
