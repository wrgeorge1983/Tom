...
class MetricsExporter:
    ...
    async def generate_metrics(self) -> bytes:
        ...
        workers_active = Gauge(
            "tom_workers_active", "Number of active workers", registry=registry
        )
        ...
        queue_depth = Gauge(
            "tom_queue_depth",
            "Jobs in queue by state (queued=waiting to be picked up, active=currently being processed)",
            ["queue", "state"],
            registry=registry,
        )
    ...
    async def _collect_queue_depth(self, queue_gauge: Gauge):
        ...
        try:
            ...
            for queue_name in queue_names:
                queued = await self.redis.llen(f"saq:{queue_name}:queued")  # type: ignore
                active = await self.redis.llen(f"saq:{queue_name}:active")  # type: ignore
                queue_gauge.labels(queue=queue_name, state="queued").set(queued)
                queue_gauge.labels(queue=queue_name, state="active").set(active)
