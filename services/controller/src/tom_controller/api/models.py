...
class CommandSpec(BaseModel):
    ...
    parse: Optional[bool] = Field(None, description="Whether to parse this command's output")
    parser: Optional[Literal["textfsm", "ttp"]] = Field(None, description="Parser to use for this command")
    template: Optional[str] = Field(None, description="Explicit template to use for parsing this command")
    include_raw: Optional[bool] = Field(None, description="Include raw output along with parsed result")
...
class SendCommandsRequest(BaseModel):
    ...
    parser: Literal["textfsm", "ttp"] = Field("textfsm", description="Default parser to use")
    include_raw: bool = Field(False, description="Default: include raw output with parsed")
    ...
    retries: int = Field(3, description="Number of times to retry on transient failures (network errors, command timeouts, etc.). Does not affect semaphore acquisition attempts.")
