...
from tom_controller.api.auth import do_auth, AuthResponse
...
async def invalidate_device_cache(
    request: Request, device_name: str, auth: AuthResponse = Depends(do_auth)
) -> dict:
    ...
    logger.info(
        f"Invalidated {deleted_count} cache entries for device {device_name} by user {auth.get('user')}"
    )
...
async def clear_all_cache(
    request: Request, auth: AuthResponse = Depends(do_auth)
) -> dict:
    ...
    logger.info(
        f"Cleared all cache: {deleted_count} entries deleted by user {auth.get('user')}"
    )
...
async def list_cache_keys(
    ...
    return {"device_filter": device_name, "count": len(keys), "keys": keys}
