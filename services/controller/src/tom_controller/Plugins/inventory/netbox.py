"""NetBox inventory plugin for Tom."""
...
class NetBoxSettings(PluginSettings):
    """
    NetBox Plugin Settings

    Config file uses: plugin_netbox_url, plugin_netbox_token, etc.
    ...
    status_filter: list[str] = []  # e.g., ["active", "staged"]
    ...
    model_config = SettingsConfigDict(
        ...
        extra="forbid",
        plugin_name="netbox",  # type: ignore[typeddict-unknown-key]
    )


class NetBoxInventoryPlugin(InventoryPlugin):
    """NetBox-based inventory plugin using pynetbox."""

    name = "netbox"
    dependencies = ["pynetbox"]
    settings_class = NetBoxSettings

    def __init__(self, plugin_settings: NetBoxSettings, main_settings: Settings):
        super().__init__(plugin_settings, main_settings)
        self.settings = plugin_settings
        self.main_settings = main_settings
        self.priority = main_settings.get_inventory_plugin_priority("netbox")
    ...
    def get_device(self, device_id: str) -> DeviceConfig | None:
        """
        Retrieve a single device by name from NetBox.

        Returns None if device not found.
        """
        try:
            # pynetbox doesn't support include parameter the same way
            # Config context is included by default in NetBox API
            device = self.nb.dcim.devices.get(name=device_id)
            ...
            print(f"Error fetching device {device_id} from NetBox: {e}")
            return None
    ...
    def get_devices(
        ...
        """
        Retrieve all devices from NetBox matching configured filters.
        ...
        try:
            ...
            print(f"Error fetching devices from NetBox: {e}")
            if "400" in str(e):
                print(
                    "Check your filter configuration (status/role/location/tag) - values must match what exists in your NetBox instance, or set to [] to disable filtering."
                )
            return []
    ...
    async def aget_device_config(self, device_name: str):
        ...
        # For now, wrap sync method (could use async later)
        import asyncio
    ...
    def list_all_nodes(self) -> list[dict]:
        ...
        try:
            ...
            # Fetch all devices matching filters - pynetbox returns list of Record objects
            devices = self.nb.dcim.devices.filter(**filters)
